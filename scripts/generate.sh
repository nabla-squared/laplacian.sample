...
DOMAIN_MODEL_DIR="$SUBPROJECTS_DIR/$DOMAIN_MODEL_MODULE_NAME"
...
APPLICATION_MODEL_DIR="$SUBPROJECTS_DIR/$APPLICATION_MODEL_MODULE_NAME"
...
generate_domain_model_plugin_src() {
  rm -rf $MODEL_PLUGIN_DIR && \
  mkdir -p $DOMAIN_MODEL_PLUGIN_DIR && \
  laplacian generate \
...
generate_backend_api_project_src() {
  rm -rf $BACKEND_API_PROJECT_DIR && \
  mkdir -p $BACKEND_API_PROJECT_DIR && \
  laplacian generate \
    --model $DOMAIN_MODEL_DIR \
